...
def lambda_handler(event, context):
    ...
    checkpath = '01250_TPCertification_ADSAToTP'
    ...
    objects = list(s3_resource.Bucket(S3_BUCKET).objects.filter(Prefix='Inbound/raw/credential/'))
    for i in objects: 
...
def lambda_handler(event, context):
    ...
    filename_to_check = "01250_TPCertification_ADSAToTP"
    # Get S3 bucket objects with the prefix specified.
    path_to_check = "Inbound/raw/credential/"
    ...
    # Get S3 bucket objects with the prefix specified.
    try:
        s3_objects = list(s3_resource.Bucket(s3_bucket_name).objects.filter(Prefix=path_to_check))
    except Exception as e:
