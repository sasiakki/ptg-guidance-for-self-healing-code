...
const ccs_service_1 = require("./services/ccs_service");
...
const processingMain = async (event) => {
    ...
    try {
        ...
        const secrets = await secretManager.getSecretValue({ SecretId: environment_type + "/b2bds/rds/system-pipelines" }).promise();
    ...
    try {
        ...
        for (const i of inputs) {
            ...
            const ccs = await entityManager.createQueryBuilder()
                ...
                .from(course_completion_1.Course_completione, 'course_completion')
                .where("course_completion.studentId = :id", { id: parsedstudentid })
                .getMany();
            student.course_completions = await ccs_service_1.ccs_service.getCourseCompletions(ccs);
            ...
            let ss, training;
            [ss, training] = bizlogics_1.Student_training_logic.evaluateStudentTraining(student, history);
            await entityManager.createQueryBuilder().update(production_person_1.prod_person)
                .set({ trainingstatus: ss.complianceStatus.toLowerCase() })
                .where("personid = :id", { id: student.id })
                .execute();
            await entityManager.createQueryBuilder().update(learner_1.learner)
                .set({ compliant: ss.complianceStatus.toLowerCase() })
                .where("personid = :id", { id: student.id })
                .execute();
        ...
        console.log(`Updating the prod.trainingrequirement table for new or updated trainings`);
