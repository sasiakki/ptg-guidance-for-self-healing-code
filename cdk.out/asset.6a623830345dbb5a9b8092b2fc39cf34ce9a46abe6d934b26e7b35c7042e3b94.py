...
if environment_type == 'prod':
    #TODO Don Not delete this comment:  As we not go live in prod we are still publishing the files to only DSHS SFTP Test folder location, After we receive signoff the change it to Prod folder location 
     sftp_root_dir = 'TEST/DS/'
    #sftp_root_dir = 'TEST/DS/'
else:
    sftp_root_dir = 'TEST/DS/'
...
try:
    #Accessing the secrets value for DSHS sftp
    sftp_response = secrets_manager_client.get_secret_value(
    SecretId="{}/b2bds/sftp/dshs".format(environment_type))
...
def write_to_sftpoutboundfilelog(outbound_files_meta_data):
    ...
    lambda_client.invoke(
        Functionname=environment_type+'-lambda-sftp-db-log-write-dshs',
...
def read_from_sftpoutboundfilelog(filecategory):
    ...
    response_for_filecategory = lambda_client.invoke(
        Functionname=environment_type+'-lambda-sftp-db-log-read-dshs',
...
outboundtypes = [
    {
        "pathfilter":"Outbound/dshsproviders/",
        "filenameformatprefix":"01250_PROVIDERS_TPTOADSA_",
        "filetypeformatsufix":"_All.txt",
        "filecategory": "dshs-providers",
        "latestprocesseddate":read_from_sftpoutboundfilelog("dshs-providers")
     }
]
...
for outboundtype in outboundtypes:
    for object_summary in s3bucket.objects.filter(Prefix=outboundtype["pathfilter"]):
        if object_summary.key.startswith(outboundtype["pathfilter"]+outboundtype["filenameformatprefix"]) and object_summary.key.endswith(outboundtype["filetypeformatsufix"]):
            ...
            filedate = re.search(r"("+outboundtype["filenameformatprefix"]+")(\d\d\d\d\d\d\d\d)(_)(\d\d\d\d\d\d)("+outboundtype["filetypeformatsufix"]+")", filename).group(2)
            ...
            tmp_filedate = datetime.strptime(filedate, '%Y%m%d').date()
...
if outbound_files_meta_data:
    try:
       ...
       raise Exception ("Failed to connect to dshs sftp {} site".format(sftp_host))
        
    print(sftp.listdir(path='TEST/DS/'))
