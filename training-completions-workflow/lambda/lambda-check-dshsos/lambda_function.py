...
def lambda_handler(event, context):
    ...
    filename_to_check = "DSHS_O&S_Completed"
    # Get S3 bucket objects with the prefix specified.
    path_to_check = "Inbound/raw/qualtrics/DSHS_O&S_Completed/"
    ...
    try:
        ...
        s3bucket = s3_resource.Bucket(s3_bucket_name)
        s3_objects = []
        for object_summary in s3bucket.objects.filter(Prefix=path_to_check):
          if object_summary.key.endswith('csv'):
            s3_objects.append(object_summary.key)
