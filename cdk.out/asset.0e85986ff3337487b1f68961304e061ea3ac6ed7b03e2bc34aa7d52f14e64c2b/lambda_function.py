...
def lambda_handler(event, context):
    ...
    filename_to_check = "CDWA-O-BG-ProviderInfo"
    # Get S3 bucket objects with the prefix specified.
    path_to_check = "Inbound/raw/cdwa/ProviderInfo/"
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "error": "Failed to retrieve Secrets Manager secret.",
            "result": False,
        }
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "error": "Failed to parse Secrets Manager secret.",
            "result": False,
        }
    ...
    try:
        ...
        return {
            "statusCode": 500,
            "error": "Failed to get S3 bucket information from Secrets Manager secret.",
            "result": False,
        }
    # Access S3 resource.
    ...
    try:
        s3_objects = list(
            s3_resource.Bucket(s3_bucket_name).objects.filter(Prefix=path_to_check)
        )
    except Exception as e:
        print(f"Failed to get S3 objects with prefix '{path_to_check}': {e}")
        return {
            "statusCode": 500,
            "error": f"Failed to get S3 objects with prefix '{path_to_check}'.",
            "result": False,
        }
