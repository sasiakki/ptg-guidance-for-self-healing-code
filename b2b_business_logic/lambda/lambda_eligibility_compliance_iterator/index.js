...
const processingMain = async (event, context, callback) => {
  ...
  try {
      ...
      const secretId = `${environment_type}/b2bds/rds/system-pipelines`
      const dbsecrets = await secretManager.getSecretValue({ SecretId: secretId }).promise();
  ...
  try {
      ...
      console.log(B2B_HOST);
      console.log(B2B_USER);
      console.log(B2B_PASSWORD);
      console.log(B2B_PORT);
