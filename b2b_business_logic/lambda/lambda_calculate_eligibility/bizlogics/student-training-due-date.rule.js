    ...
    getContinuingEducationDueDate(student, category) {
        ...
        if (isFirstCE) {
                        ...
                        if (credential) {
                            if (credential.status === consts_1.Constants.credential_status.ACTIVE) {
                                if (_.includes([consts_1.Constants.credential_type.HCA.CODE, consts_1.Constants.credential_type.NAC.CODE], credential.name)) {
                                    const activeCredentialFirstIssuance = moment(credential.firstIssuance_date).utc().startOf('day');
                                    if (process.env.IS_LOCAL) {
                                        console.log('\n**************************');
                                        console.log('activeCredential \t', credential.name);
                                        console.log('activeCredentialFirstIssuance \t', activeCredentialFirstIssuance.toDate());
                                        console.log('**************************\n');
                                    }
                                    if (activeCredentialFirstIssuance.isAfter(moment(nextDOB).utc().add(-1, 'year'))) {
                                        if (process.env.IS_LOCAL)
                                            console.log('!!! HCA|NAC credential, issuance date is within 365 days of next birthday,  providers next birthday + 1 year !!!\n');
                                        return nextDOBplusOne;
                                    }
                                    else {
                                        if (process.env.IS_LOCAL)
                                            console.log('!!! HCA|NAC credential, issuance date is on or before 365 days of next birthday,  providers next birthday !!!\n');
                                        return nextDOB;
                                    }
                                }
                                ...
                                const expiredCredentialFirstCENextDOB = this.getCredentialExpirationCEDueDate(student, credential.expiration_date);
                                if (process.env.IS_LOCAL)
                                    console.log('!!! expired | pending credential, use birthday of expiration date year as due date !!!\n');
                                return expiredCredentialFirstCENextDOB;
                            }
                            else {
                                return undefined;
                            }
            ...
            let dueDate = (lastCompletedTraining.due_date)
                ...
                : nextDOB;
            if (lastCompletedTraining.due_date && student.student_status_set.isRehire === true) {
                if (category === consts_1.Constants.work_category.ADULT_CHILD) {
                ...
                if (process.env.IS_LOCAL)
                    console.log('!!! rehire duedate !!!\t', dueDate);
            }
            return dueDate;
        }
    }
