...
delta_personids = spark.sql("""SELECT * FROM person WHERE recordmodifieddate > (SELECT MAX(lastprocesseddate) FROM lastprocessed WHERE processname = 'docebo-update-user-data-outbound' AND success = '1') 
OR personid IN (SELECT personid from trainingrequirement WHERE trackingdate = CURRENT_DATE) 
OR personid in (select distinct(personid) from trainingrequirement where recordmodifieddate > (SELECT MAX(lastprocesseddate) from lastprocessed WHERE processname = 'docebo-update-user-data-outbound' AND success = '1'))""")
...
trainingrequirement_select = trainingrequirement_df.select(col('trainingprogramcode'),col("trainingprogram"),col("trainingid"),col("completeddate"),col("trackingdate"),col("duedate"),col("duedateextension"),col("requiredhours"),col("earnedhours"),col("transferredhours"),col("status").alias("trainingrequirement_status"),col("isrequired"),col("archived"),col("personid"))\
                                ...
                                .filter(col("completeddate").isNull())\
...
update_userinfo_df = current_emp_result.join(trainingrequirement_active,current_emp_result.personid == trainingrequirement_active.personid,"leftouter").drop(trainingrequirement_active.personid)

update_userinfo_df = update_userinfo_df.withColumn("Active", when(((col("person_status") == 'Terminated') & (col("trainingrequirement_status") == 'active')),0).when((col("person_status") == 'Terminated') & (col("trainingrequirement_status") == 'closed'),0).when((col("person_status") == 'Terminated' ) & (col("trainingrequirement_status").isNull()),0).otherwise(1))\

update_userinfo_df  = update_userinfo_df\
    .withColumn("futuretrackingdate",when( col("futuretrackingdate") > current_timestamp(),1).when( col("futuretrackingdate") <= current_timestamp(),0 ).when( length("futuretrackingdate") == 0,0 ).otherwise(0) )\
