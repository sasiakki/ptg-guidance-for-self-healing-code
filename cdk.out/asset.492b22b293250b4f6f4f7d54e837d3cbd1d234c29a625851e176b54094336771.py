...
from pyspark.sql.functions import col,current_date,coalesce,concat,rank,concat_ws,when,collect_set,lit,to_date,date_format,initcap,lower,upper, rand, floor, current_timestamp, regexp_replace, length
...
S3_BUCKET = s3_secrets_docebo['datafeeds']
...
#Captruting employmentreleationship
employmentreleationship_df = glueContext.create_dynamic_frame.from_catalog(database=catalog_database, table_name=catalog_table_prefix+"_prod_employmentrelationship").toDF()
...
delta_personids = spark.sql("""SELECT * FROM person WHERE personid IN (SELECT personid FROM trainingrequirement WHERE recordcreateddate > (SELECT MAX(lastprocesseddate) from lastprocessed WHERE processname = 'docebo-new-training-file-data-outbound' and success = '1') AND status = 'active' and isrequired = '1' )""")


delta_trainingrequirements = spark.sql("""SELECT * FROM trainingrequirement WHERE recordcreateddate > (SELECT MAX(lastprocesseddate) from lastprocessed WHERE processname = 'docebo-new-training-file-data-outbound' and success = '1') AND status = 'active' and isrequired = '1'""")
...
employmentreleationship_select = employmentreleationship_df.filter(col("role") == "CARE").filter(col("createdby")!="ZenithLegacy").filter(col("empstatus") == "Active").select(col("employerid").alias("employer_id"),col("empstatus"),col("personid"))
...
current_employer= trust_current_employer_trust.join(employmentreleationship_select,trust_current_employer_trust.employerid == employmentreleationship_select.employer_id,"leftouter").groupBy("personid").agg(concat_ws(";" , collect_set("employername")).alias("Current Employer"),concat_ws(",",collect_set(when(coalesce(col("employer_id"),lit(0)).isin(["103","426"]),"IP").otherwise("AP"))).alias("providertype")).select(col("personid").alias("personid"),col("Current Employer"), col("providertype"))
...
trainingrequirement_active = trainingrequirement_select.select(col('trainingprogramcode'),col("trainingprogram"),col("trainingid"),col("completeddate"),col("trackingdate"),col("trackingdate").alias("futuretrackingdate"),col("duedate"),col("duedateextension"),col("requiredhours"),col("earnedhours"),col("transferredhours"),col("trainingrequirement_status"),col("personid"),rank().over(training_ranking_window).alias("training_rank")).filter(col("training_rank") == 1)\
                                    .withColumn("trackingdate", date_format(to_date(col("trackingdate"), "yyyy-mm-dd"),"MM/dd/yyyy"))\
                                    .withColumn("duedate", date_format(to_date("duedate", "yyyy-mm-dd"),"MM/dd/yyyy"))\
                                    .withColumn("duedateextension", date_format(to_date(col("duedateextension"), "yyyy-mm-dd"),"MM/dd/yyyy"))\
                                    .withColumn("completeddate", date_format(to_date(col("completeddate"), "yyyy-mm-dd"),"MM/dd/yyyy"))

new_training_info_df = current_emp_result.join(trainingrequirement_active,current_emp_result.personid == trainingrequirement_active.personid,"leftouter").drop(trainingrequirement_active.personid)

new_training_info_df = new_training_info_df.withColumn("Active", when(((col("person_status") == 'Terminated') & (col("trainingrequirement_status") == 'active')),0).when((col("person_status") == 'Terminated') & (col("trainingrequirement_status") == 'closed'),0).when((col("person_status") == 'Terminated' ) & (col("trainingrequirement_status").isNull()),0).otherwise(1))\


new_training_info_df = new_training_info_df.withColumnRenamed('status',"Employment Status")\
.drop(col("value"))

new_training_info_df = new_training_info_df.withColumn("futuretrackingdate",when( col("futuretrackingdate") > current_timestamp(),1).when( col("futuretrackingdate") <= current_timestamp(),0 ).when( length("futuretrackingdate") == 0,0 ).otherwise(0) )\
