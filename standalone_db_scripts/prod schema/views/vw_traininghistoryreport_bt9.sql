-- View: prod.vw_traininghistoryreport_bt9

-- DROP VIEW prod.vw_traininghistoryreport_bt9;

CREATE OR REPLACE VIEW prod.vw_traininghistoryreport_bt9
 AS
 SELECT DISTINCT th.personid,
    ...
    th.coremod1 + th.coremod2 + th.coremod3 + th.coremod4 AS completedmodules,
    4 - (th.coremod1 + th.coremod2 + th.coremod3 + th.coremod4) AS pendingmodules,
    tr.hourscompleted,
    tr.reqhours::double precision - tr.hourscompleted AS remaininghours,
        CASE
            WHEN (th.coremod1 + th.coremod2 + th.coremod3 + th.coremod4) = 0 THEN 'Not Started'::text
            WHEN (th.coremod1 + th.coremod2 + th.coremod3 + th.coremod4) = 4 THEN 'Completed'::text
   ...
   FROM ( SELECT a.personid,
           ...
           FROM ( SELECT transcript.personid,
                    transcript.courseid,
                        CASE
                            WHEN transcript.coursename::text ~~ 'The Caregiver and Client Experience%'::text THEN 1
                            ELSE 0
                        END AS coremod1,
                        CASE
                  ...
                  WHERE "left"(transcript.courseid::text, 3) = '203'::text
                  GROUP BY transcript.personid, transcript.coursename, transcript.courseid) a
     ...
     LEFT JOIN ( SELECT trainingrequirement.personid,
          ...
          WHERE trainingrequirement.trainingprogramcode::text = '203'::text
          GROUP BY trainingrequirement.personid, trainingrequirement.trainingprogramcode, trainingrequirement.trackingdate) tr ON th.personid = tr.personid AND tr.trainingprogramcode::text = '203'::text
     ...
     LEFT JOIN ( SELECT transcript.transcriptid,
           ...
           FROM prod.transcript
          WHERE transcript.completeddate::date < '2020-12-17'::date) legacy ON th.personid = legacy.personid AND "left"(legacy.courseid::text, 3) = '203'::text
