...
class Student_bt_rehire_rule {
    ...
    static processTrainings(student, btTrainings) {
        ...
        if (student.student_status_set.isRehire
            && student.employments
            && moment(consts_1.Constants.REHIRE_MIN).utc().startOf('day').isSameOrBefore(moment(student.employments[0].rehire_date).utc().startOf('day'))
            && moment(consts_1.Constants.REHIRE_MAX).utc().startOf('day').isSameOrAfter(moment(student.employments[0].rehire_date).utc().startOf('day'))) {
            if (!(0, util_1.isNullOrUndefined)(btTrainings)) {
                _.forEach(btTrainings, tr => {
                    if (_.includes(helper_1.Helper.BT_CODES, tr.training_code)
                        ...
                        && tr.tracking_date
                        && moment(tr.tracking_date).utc().startOf('day').isBefore(moment(consts_1.Constants.REHIRE_MIN).utc().startOf('day'))) {
