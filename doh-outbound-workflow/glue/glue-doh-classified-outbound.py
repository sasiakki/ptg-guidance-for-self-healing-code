...
spark.sql("select * from ( select tr.personid AS btpersonid ,tr.duedate AS btduedate ,tr.trainingprogram AS bttraining ,tr.completeddate AS btcompleted ,tr.earnedhours AS btearnedhours ,tr.trackingdate AS bttrackingdate ,tr.trainingprogramcode as bttrainingprogramcode ,ROW_NUMBER() OVER(PARTITION BY tr.personid ORDER BY tr.trainingid desc ,tr.completeddate desc) as trRank from  trainingrequirement tr WHERE tr.trackingdate >= '2013-09-01 00:00:00' and tr.trainingprogramcode = '201' ) tr where tr.trRank = 1").createOrReplaceTempView("bt70trainingcompletedcte_tr")
...
pandas_df.to_csv("s3://"+S3_BUCKET+"/Outbound/doh/classified/Classified_DOH_BT75_Students_"+suffix+"_1.txt", header=True, index=None, sep='|')

headerdf = spark.sql("""select 'bg.seiu775.datastore' as ChangeOrigin, personid as DatastoreId, 'CREATE'  as ChangeType, 
'' as RecordID, 'Certification_Benefit_Fund__c' as EntityName from doh_outbound_df """)
headerdf.show()

#Converting dataframe to pandas df
benefits_certification_df = doh_outboundDF.toPandas()
...
for i in range(0,benefits_certification_df.shape[0]):
    row_s=benefits_certification_df.iloc[i]
    ...
    Entries=[
        {
            ...
            'Detail': payload,
            'EventBusName': 'arn:aws:events:us-west-2:259367569391:event-bus/default'
        }
    ])
